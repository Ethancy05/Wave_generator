...
void Draw_Menu(void)
{
	...
	OLED_Clear();
	for(t = 0;t<4;t++)
	OLED_ShowChinese(t*16,0,t,16);
	OLED_ShowString(0,18," Regulator",12);
	for(t = 4;t<6;t++)
	OLED_ShowChinese(82+(t-4)*16,0,t,16);
	OLED_ShowString(90,18,"SET",12);
	for(t = 6;t<11;t++)
	OLED_ShowChinese((t-6)*16,32,t,16);
	...
	for(t = 11;t<13;t++)
	OLED_ShowChinese(16+(t-11)*16,48,t,16);
	for(t = 13;t<17;t++)
	OLED_ShowChinese(32+(t-11)*16,48,t,16);
	OLED_Refresh();
	while(1)
	{
			if(KEY_Value != 255)
			{
				...
				{
					case 0://OK是返回键
						LED_RED = !LED_RED;delay_ms(5);LED_RED = !LED_RED;//闪灯
						//返回
						goto X;
					case 1://SET是校准键
						LED_GREEN = !LED_GREEN;delay_ms(5);LED_GREEN = !LED_GREEN;//闪灯
						...
						break;
					case 2://剩下两个键没用到
						LED_BLUE = !LED_BLUE;delay_ms(5);LED_BLUE = !LED_BLUE;
						goto X;
					case 3:
						LED_RED = !LED_RED;LED_GREEN = !LED_GREEN;LED_BLUE = !LED_BLUE;delay_ms(5);LED_RED = !LED_RED;LED_GREEN = !LED_GREEN;LED_BLUE = !LED_BLUE;
						goto X;
					default: break;
				}
				KEY_Value = 255;	
...
void DrawOscillogram(void)//画波形图
{
	...
	if(KEY_Value != 255)
	{
		...
		{
			case 0:
				LED_RED = !LED_RED;LED_GREEN = !LED_GREEN;LED_BLUE = !LED_BLUE;delay_ms(10);LED_RED = !LED_RED;LED_GREEN = !LED_GREEN;LED_BLUE = !LED_BLUE;
				if(menu == 3)//进入menu功能
				{
					KEY_Value = 255;
					Draw_Menu();
				...
				break;
			case 1:
				LED_GREEN = !LED_GREEN;LED_RED = !LED_RED;delay_ms(10);LED_GREEN = !LED_GREEN;LED_RED = !LED_RED;
				menu++;if(menu>3)menu=0;//改变菜单位置
				break;
			case 2:
				LED_BLUE = !LED_BLUE;LED_RED = !LED_RED;delay_ms(10);LED_BLUE = !LED_BLUE;LED_RED = !LED_RED;
				//改变属性+
				if(menu == 1)
				{
				...
				break;
			case 3:
				LED_BLUE = !LED_BLUE;LED_GREEN = !LED_GREEN;delay_ms(10);LED_BLUE = !LED_BLUE;LED_GREEN = !LED_GREEN;
				//改变属性-
				if(menu == 1)
				{
